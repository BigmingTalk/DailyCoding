...
/**
 * Created by bigming on 16/9/7.
 * 题目: 调制搜索二叉树中两个错误地节点,即原来搜索二叉树中的两个节点换了,
 *      要求找出错误地两个节点.
 *      进阶问题: 通过结构的方式更换两个错误节点,变回原来的搜索二叉树,
 *      但不是改变值的方式.
 *
 * 难度: **
 * 进阶难度: ****
 * 思路: 找到错误节点只需要用中序的方式遍历,会出现两次逆序,第一次逆序选较大
 *      的,第二次逆序选较小的,然后这两个节点就是要求的,如果只出现一次逆序,
 *      那就说这两个节点.
 *      对于进阶问题, 首先要找出两个错误节点的头结点,然后要考虑三个问题,即
 *      错误节点是否为head节点,连个错误节点是否相邻,以及两个错误节点分别是
 *      其父节点的左孩子还是右孩子,最后一共得到14种情况.
 *
 */
public class Problem_10_RecoverBST_me {
